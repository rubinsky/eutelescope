...
//  int dz = 20;   // [mm] z spacing
    ...
    double resx =  3.00e-03;//3.0E-6; // [mm] intrinsic resolution cols, 00 deg
    double resy =  3.00e-03;//3.0E-6; // [mm] intrinsic resolution rows, 00 deg
...
double distmax=160.;
  ...
  for(double ddut = distmin; ddut<=distmax; ddut=ddut+10 ) 
  {

  for(double dz = distmin; dz<=distmax; dz=dz+10 )
  {
  if( abs(ddut/dz -1) > 0.001) continue;
  for(double dz2 = distmin; dz2<=distmax; dz2=dz2+10  )
  {
  if( abs(dz2/dz -1) > 0.001) continue;

  for(double dkapton = 25.0; dkapton<=25.; dkapton*=2.)
  {
...
float JIGthickness     = 5.; // 15.
float JIGM26toRight    = 2.0;
float JIGM26toLeft     = JIGthickness - JIGM26toRight; // 13. distance from the left side to Mimosa26

          step = JIGM26toRight; // 2 mm to the kapton foil // ignore the air in 2 mm !!
          dummy = BookPlane( traj, meas, measPrec, scat, wscatKAPTON, sPoint, false, false, iz[i] - iz[kplane] );


          // continue if i > 0
          step = iz[i] - iz[i-1] - JIGM26toLeft - JIGM26toRight ; // JIG thickness
          ...
          step = JIGM26toLeft; // distance within the JIG from the kapton to Mimosa26
