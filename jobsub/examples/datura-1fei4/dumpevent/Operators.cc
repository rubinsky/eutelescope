...
namespace UTIL{
    ...
    //   # TrackState
    //============================================================================
    const std::string& header(const EVENT::TrackState *){ //hauke
        static std::string _h(" [   id   ] |          |    d0    |  phi     | omega    |    z0     | tan lambda|   reference point(x,y,z)        |          |          |          |        | Location \n");
        return _h;
    }

    const std::string& tail(const EVENT::TrackState *){ //hauke
        static std::string _t("------------|----------|----------|----------|----------|-----------|-----------|---------------------------------|----------|----------|--------- | -------| ------    \n");
        return _t;
    }
/*
...
*/
    std::ostream& operator<<( std::ostream& out, const UTIL::lcio_short<EVENT::TrackState>& sV){
        ...
        out << " |" << dec << setw(9) << " ";
        out << " |" << dec << setw(9) << " ";
        out << " |" << noshowpos << setw(7) << " " ; 
        out << " | " << noshowpos << setw(5) << trk->getLocation() ; 
//        out << endl;
/*
//        out << noshowpos << 
        out << " [" << setfill('0') << setw(8) << dec<< trk->id() << "] ";
        ...
        out << " |" << trk->getTanLambda();
        out << " |(" << setprecision(2) ;
        out << trk->getReferencePoint()[0] << ", " << trk->getReferencePoint()[1] << ", " <<trk->getReferencePoint()[2];
        out << " ) ";
        out << endl;
*/
/*
        out << " cov matrix: " << showpos << scientific << setprecision(2) << setw( 4) << setfill(' ')  ;
...
//       out << lcio_long(part,NULL);
        out << UTIL::lcio_short< EVENT::TrackState> ( &part,NULL) ;
    ...
    const std::string& header(const EVENT::Track *){ //hauke
        static std::string _h(" [   id   ] |   type   |    d0    |  phi     | omega    |    z0     | tan lambda|   reference point(x,y,z)        |    dEdx  |  dEdxErr |   chi2   |  ndf   | states \n");
        return _h;
    }
    ...
    std::ostream& operator<<( std::ostream& out, const UTIL::lcio_short<EVENT::Track>& sV){ //hauke
        ...
        out << " |" << trk->getChi2();
        out << " |" << noshowpos << setw(7) << trk->getNdf() ; 
        out << " | " << noshowpos << setw(5) << trk->getTrackStates().size() ; 
        out << endl;

/*
        out << " errors: "  << showpos << scientific << setprecision(2) << setw(4) << setfill(' ')  << endl ; 
...
/*
	  ...
	  out << dec << setfill('0')
//           << "[" << setw(8)  << dec << (  tracks[l]  ?   tracks[l]->id()  : 0 ) << "]" ;
	      <<  tracks[l] ;
//	  if( l != N-1 ) out << ", ";
	}
        out << endl;
*/ 
        const TrackStateVec& trackState = trk->getTrackStates() ;

        out << " tracks states (id): " << trackState.size() << endl ;

        for(unsigned int l=0,N=trackState.size(); l<N ;l++){
	  
	  out << dec << setfill('0')  << *(trackState[l]) ;
	}
        out << endl;
 ...
 //        out << lcio_long(part,NULL);
        out << UTIL::lcio_short<EVENT::Track> (&part,NULL);
        return(out);
    }
